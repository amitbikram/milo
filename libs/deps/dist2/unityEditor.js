...
class Bt {
  constructor(t, e = {}) {
    ...
    r(this, "ondemandLoad", async () => {
      this.canvasLoaded || (this.fabric = await import("./fabric-T-x1IxqJ.js").then((t) => t.f), this.filters = U.init(), this.loadCanvas(), this.state = new pt(this.canvas, this.config), this.controlManager.canvasLoaded.forward());
    });
...
class Ft {
  checkMountType(t) {
    if (t.tagName === "IMG") {
      this.src = t.src;
      const e = t.clientHeight, i = t.clientWidth, s = document.createElement("div");
      return s.style.height = `${e}px`, s.style.width = `${i}px`, s.classList.add("editor-wrapper"), W(t) && (s.style.position = "relative"), t.parentNode.insertBefore(s, t), s.appendChild(t), s;
    } else if (t.tagName === "PICTURE") {
      const e = t.querySelector("img");
      this.src = e.src;
      const i = e.clientHeight, s = e.clientWidth, n = document.createElement("div");
      return n.style.height = `${i}px`, n.style.width = `${s}px`, n.classList.add("editor-wrapper"), W(t) && (n.style.position = "relative"), t.parentNode.insertBefore(n, t), n.appendChild(t), n;
    } else {
      const e = W(t);
...
let Q = 100, V = null, tt = null, u = null;
const Et = {
  load: "lazy"
};
async function qt() {
  await J("/assets/ffmpeg/package/dist/umd/ffmpeg.js"), await J("/assets/util/package/dist/umd/index.js"), V = FFmpegUtil.fetchFile, tt = FFmpegWASM.FFmpeg, u = new tt(), u.on("log", ({ message: a }) => {
    console.log(a);
...
async function kt(a, t) {
  ...
  const [o, h] = S(null), [p, g] = S({}), [q, C] = S([]), [f, b] = S(!1), [T, j] = S(null), [Tt, G] = S(!1), [k, et] = S(0), [H, it] = S(10);
  a.insertAdjacentHTML("afterend", xt), document.querySelector("#import-btn").addEventListener("change", st), a.addEventListener("loadedmetadata", at);
  ...
  async function N() {
    ...
    try {
      ...
      document.querySelector("video").src = x, j(x);
    } catch (v) {
      console.log(v);
    } finally {
      G(!1);
  ...
  async function z(l) {
    ...
    h(v);
    const x = await nt(v), B = d(et), M = d(it), $ = !1, E = { duration: 60 };
    Mt({
      thumbNails: x,
      rEnd: H(),
      rStart: k(),
